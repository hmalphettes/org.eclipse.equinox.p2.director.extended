 ...
 */
public class ForkedDirectorApplication implements IApplication {
	...
	//added here.
	private static final CommandLineOption OPTION_ADD_SOURCES = new CommandLineOption(new String[] {"-addSources"}, null, "download source bundles"); //$NON-NLS-1$
	...
	//added here:
	private boolean addSourcesBundles = false;
	...
	/**
	 * Easy way to override the planner by extending the application.
	 * @return
	 */
	...
	private void performProvisioningActions() throws CoreException {
		...
		try {
			...
			if (addSourcesBundles) {
				context.setProperty(WITH_BUNDLE_SOURCES, String.valueOf(addSourcesBundles));
			}
			ProfileChangeRequest request = buildProvisioningRequest(profile, installs, uninstalls);
			if (addSourcesBundles) {
				request.setProfileProperty(WITH_BUNDLE_SOURCES, String.valueOf(addSourcesBundles));
			}
	...
	private void printRequest(ProfileChangeRequest request) {
		...
		String withSources = "";
		if (Boolean.TRUE.toString().equals(request.getProfileProperties().get(WITH_BUNDLE_SOURCES))) {
			withSources = " with bundle sources that can be found ";
		}
		for (IInstallableUnit added : toAdd) {
			printMessage(NLS.bind(Messages.Installing + withSources, added.getId(), added.getVersion()));
		}
	...
	public void processArguments(String[] args) throws CoreException {
		...
		for (int i = 0; i < args.length; i++) {
			...
			//added here:
			if (OPTION_ADD_SOURCES.isOption(opt)) {
				addSourcesBundles = true;
				continue;
			}
